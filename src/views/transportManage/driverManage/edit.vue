...
export default {
  ...
  methods: {
    ...
    updateInfo() {
      request({
        url: '/basic/orgdriver/info',
        method: 'get',
        params: {
          driverId: this.driverId
        }
      }).then(res => {
        this.driverFormInfo.data = res.data
      })
    },
    // 上传照片
    ...
    handleSave() {
      this.driverFormInfo.ref.validate(valid => {
        if (valid) {
          delete this.driverFormInfo.data.createTime
          delete this.driverFormInfo.data.updateTime
          this.driverFormInfo.data.driverLicenseIssueDate = this.driverFormInfo.data.driverLicenseIssueDate.split(' ')[0]
          request({
            url: '/basic/orgdriver/modify',
            method: 'put',
