      ...
      >
        ...
        <el-table-column align="center" fixed="right" label="操作" slot="append" width="250">
          <template slot-scope="scope">
            ...
            <el-button @click="handleClickCheckOrder(scope.row.id)" type="primary">核销</el-button>
            <el-button :disabled="scope.row.status !== 'NONE'" @click="handleDelete(scope.row.id)" type="danger">删除</el-button>
          </template>
        </el-table-column>
      </custom-table>
    </div>
    <el-dialog :visible.sync="isAdd" @close="handleCloseAddOrder" title="添加收款订单" width="50%">
      <custom-form
...
import { recPayOrderFormInfo, listTypeInfo } from './data.js'
