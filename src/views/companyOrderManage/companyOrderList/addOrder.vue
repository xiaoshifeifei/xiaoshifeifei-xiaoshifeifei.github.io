<template>
  <div>
    <el-card>
      <div slot="header">
        <span>添加运单</span>
        <el-button @click="changeShowPage('list')" style="float: right; margin-top: -5px" type="primary">返回</el-button>
      </div>
      <!-- 基本信息 -->
    ...
    <!-- 添加地址 -->
    <el-dialog title="添加地址" :visible.sync="isAddAddress" @close="handleCloseAddress">
      <addAddress ref="addAddress" @on-close="handleCloseAddress" />
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    handleChangeShipperUnit(data) {
      this.listTypeInfo.addressList.forEach(item => {
        if (item.addrId == data) {
          this.sendFormInfo.data.shipperId = item.addrId,
          this.sendFormInfo.data.shipperUnit = item.addrUnit,
          this.sendFormInfo.data.shipperPerson = item.addrName,
          this.sendFormInfo.data.shipperTel = item.addrTel,
          this.sendFormInfo.data.shipperAddress = item.addrArea,
          this.sendFormInfo.data.shipperAddressCode = item.addrCode,
          this.sendFormInfo.data.shipperAddressDetial = item.addrDetail,
          this.areaForm.sendArea = item.addrCode.split(',')
        }
      })
    },
    handleChangeReceiverUnit(data) {
      this.listTypeInfo.addressList.forEach(item => {
        if (item.addrId == data) {
          this.getFormInfo.data.receiverId = item.addrId,
          this.getFormInfo.data.receiverUnit = item.addrUnit,
          this.getFormInfo.data.receiverPerson = item.addrName,
          this.getFormInfo.data.receiverTel = item.addrTel,
          this.getFormInfo.data.receiverAddress = item.addrArea,
          this.getFormInfo.data.receiverAddressCode = item.addrCode,
          this.getFormInfo.data.receiverAddressDetial = item.addrDetail,
          this.areaForm.getArea = item.addrCode.split(',')
        }
      })
    },
